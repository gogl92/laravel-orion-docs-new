...
This will introduce `restore` and `batchRestore` endpoints. To learn how to permanently delete a resource via API (force delete), take a look at the related [Query Parameters](/guide/query-parameters#soft-deletes) section.
...
For one-to-one relationships, Orion provides 4 endpoints (basically endpoints for CRUD operations): `store`, `show`, `update`, `destroy`
...
<br/><br/>`hasOne`, `hasOneThrough`, `morphOne`, `belongsTo`, and `morphTo` relations do not require the related resource key.
::
## One to Many
...
color: amber
---
Do not forget to define fillable pivot fields as described in [Fillable pivot fields and casting](/guide/relationships#setting-up-controller) section, if you have additional fields on pivot table, otherwise `attach`, `sync`, `toggle` and `updatePivot` endpoints may not work as expected for these fields.
::

### Attaching

Many-to-many relation resource provides `attach` endpoint to attach one or multiple related models to another model. For details on how attaching/detaching of related models works in Laravel, take a look at [Attaching / Detaching](https://laravel.com/docs/eloquent-relationships#updating-many-to-many-relationships) section in Laravel Documenation.
...
Many-to-many relation resource provides `pivot` endpoint to update pivot row of one of the related models. For details on how pivot row is updated, take a look at [Updating A Record On A Pivot Table](https://laravel.com/docs/eloquent-relationships) section in Laravel Documentation.
...
Just like [model resources](/guide/models#customizing-keys), relation resources are using primary key to fetch resources from the database.
...
It is possible, same as in [model controllers](/guide/models#customizing-queries), to redefine Eloquent queries for each endpoint. The only major difference is that each endpoint in relation controller also has "build" and "run" methods for fetching relation's parent model.
